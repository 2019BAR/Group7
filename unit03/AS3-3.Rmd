...
max(summary(CPSD$Industry))
table(CPSD$Industry)
#sort(summary(CPSD$Industry))
#
```
...
```{r}
table(CPSD$Region, is.na(CPSD$Married))

table(CPSD$Sex, is.na(CPSD$Married))

table(CPSD$Age, is.na(CPSD$Married))

table(CPSD$Citizenship, is.na(CPSD$Married))
...
```{r}
#依據上表，有三個(District of Columbia，New Jersey，Rhode Island )
#
```

__§ 2.4__ Which region of the United States has the largest proportion of interviewees living in a non-metropolitan area?
```{r}
table(CPSD$Region, is.na(CPSD$MetroAreaCode))
#midwest=10674/20010=0.533
#northeast=0.275897688
#south=0.312067276
#west=0.322161559
```
...
```{r}
tapply(is.na(CPSD$MetroAreaCode), CPSD$State, mean)
#Wisconsin  
#
```
...
```{r}
sort(tapply(is.na(CPSD$MetroAreaCode), CPSD$State, mean))
...
```{r}
sort(tapply(CPSD$Hispanic, CPSD$MetroArea, mean))
#
#
```
...
```{r}
sort(tapply(CPSD$Race == "Asian", CPSD$MetroArea, mean))
#
#
```
...
```{r}
sort(tapply(CPSD$Education == "No high school diploma", CPSD$MetroArea, mean, na.rm=TRUE))
#
#
```
...
```{r}
CPSD = merge(CPSD, Country, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE)
summary(CPSD)
#
#
```
...
```{r}
table(CPSD$MetroArea == "New York-Northern New Jersey-Long Island, NY-NJ-PA", CPSD$Country != "United States")
#1668/(1668+3736)=0.309
#
```
...
```{r}
sort(tapply(CPSD$Country == "India", CPSD$MetroArea, sum, na.rm=TRUE))
...
In Brazil?
```{r}
sort(tapply(CPSD$Country == "Brazil", CPSD$MetroArea, sum, na.rm=TRUE))
...
In Somalia?
```{r}
sort(tapply(CPSD$Country == "Somalia", CPSD$MetroArea, sum, na.rm=TRUE))
