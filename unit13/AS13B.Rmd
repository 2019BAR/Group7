...
```{r}
glm1 = glm(PoorCare ~ InpatientDays + ERVisits + OfficeVisits + Narcotics + DaysSinceLastERVisit + Pain + TotalVisits + ProviderCount +
      MedicalClaims + ClaimLines + StartedOnCombination + AcuteDrugGapSmall        , TR, family=binomial)
```

&emsp; 【A】 分別畫出`Training`和`Testing`的`DPP`<br>
```{r}
#Training
pred_TR = predict(glm1, newdata=TR, type="response")
...
#Testing
pred_TS = predict(glm1, newdata=TS, type="response")
...
```{r}
cmx = table(Acture=TR$PoorCare, Predict=pred_TR > 0.5)
cmx2 = table(Acture=TS$PoorCare, Predict=pred_TS > 0.5)
sapply(list(Train=cmx, Test=cmx2), A2x2)
```
